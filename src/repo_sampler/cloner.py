...
async def clone_repo(url: str, dest: Path, timeout: int = 900) -> Path:
    if dest.exists():
        rc, out, _ = await _run_git(
            ["rev-parse", "--is-shallow-repository"], cwd=dest, timeout=timeout
        )
        ...
        if rc == 0:
            logger.warning(f"Cached clone is shallow; fetching full history: {dest}")
            ...
            if rc == 0 and _is_local_git_source(url):
                rc, _, _ = await _run_git(
                    [
                        "fetch",
                        "--quiet",
            ...
            check_rc, check_out, _ = await _run_git(
                ["rev-parse", "--is-shallow-repository"], cwd=dest, timeout=timeout
            )
            if rc == 0 and check_rc == 0 and check_out.strip() == "false":
                return dest

        logger.warning(f"Cached clone is unusable; cloning it again: {dest}")
        cleanup_repo(dest)
    ...
    proc = await asyncio.create_subprocess_exec(
        ...
        # and checkout_latest_branch can move off an empty/README-only default
